...
a 5-byte header (``>Lc`` = big-endian ``uint32`` byte length + 1-byte
serialization tag) followed by the payload. The tag is one of ``n`` (raw
bytes), ``u`` (UTF-8 string), ``j`` (JSON), or ``d`` (dill pickle). Payloads
are framed on the **payload byte length** (UTF-8 bytes for the ``j``/``u`` text
frames; raw bytes for the ``n`` and ``d`` binary frames — the ``d`` dill
pickles are not UTF-8), and the length field rejects payloads ≥ 4 GiB rather
...
``("localhost", 0)``; the extension listener uses ``init(-1, true, -1)``),
bound to **random ephemeral ports** exchanged out-of-band via the profile
directory, and **not reachable from web content or off-host**. They are,
however, **unauthenticated** — there is no per-connection token, and the first
client message is just a name string.
...
``d`` (dill) frame is deserialized via ``dill.loads``, an
**arbitrary-code-execution primitive** in the StorageController process. This
is **acceptable for single-tenant deployments** (the typical measurement host)
but a **real risk on shared / multi-tenant hosts**. The wire protocol is
intentionally left unchanged; hardening (auth token / dropping dill on the
wire) is tracked in
`#1179 <https://github.com/openwpm/OpenWPM/issues/1179>`_.
