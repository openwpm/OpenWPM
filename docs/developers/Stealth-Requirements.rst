...
the stealth direction because legacy does not reliably exhibit them in this build
...
Frame-protection detection vectors (D10)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The vectors above probe the **fingerprinting** instrument. The stealth build
also inherits the legacy **frame-protection / DOM-hook** layer
(``Extension/src/stealth/index.ts``), which installs page-facing ``Proxy``
objects on page prototypes (``Node.prototype.appendChild``, ``document.write``,
``window.open``, and the ``HTMLIFrameElement.prototype.contentWindow`` getter)
so that dynamically-created frames are instrumented within the parent's context
(this is the mechanism behind X3). These hooks are a distinct surface with their
own ``toString`` / arity / name / cross-realm tells, exercised by the new
``TestStealthDetectability`` rows in ``test/test_pages/stealth_detection.html``.
All 16 are **stealth-only** (``legacy_detectable=None``): there is no paired
legacy control because legacy has no such layer to contrast against. The
frame-protection hooks are built only into the stealth bundle
(``Extension/webpack.config.js`` maps ``src/stealth/index.ts`` to ``stealth.js``;
the legacy entry ``src/content.ts`` never imports them), so these rows
characterise the stealth instrument's own residual surface.
...
.. list-table::
   ...
   * - **D10-fp-appendChild-xrealm**
     - fresh-realm ``toString`` on hooked ``appendChild`` is ``[native code]``
   ...
   * - **D10-fp-docwrite-xrealm**
     - fresh-realm ``toString`` on hooked ``document.write`` is native
   ...
   * - **D10-fp-window-open-xrealm**
     - fresh-realm ``toString`` on hooked ``window.open`` is native
   ...
   * - **D10-fp-contentwindow-getter-xrealm**
     - fresh-realm ``toString`` on hooked ``contentWindow`` getter is native
     - ``fp_contentwindow_xrealm_native``

.. note::

   **D10 status — asserted but UNVERIFIED, and test-bounded.** These 16 vectors
   cover the inherited ``index.ts`` frame-protection ``Proxy`` surface. Unlike
   the fingerprinting instrument, this layer **does** patch the **main-realm**
   ``Function.prototype.toString`` and runs a realm "treadmill" to re-apply that
   patch in newly-reachable realms — the exact limitation recorded in
   ``adr/0001-retain-legacy-js-instrument.rst``. The ``*-xrealm`` vectors
   specifically probe the **Wall-1 cross-realm ``toString`` tell**: whether a
   ``toString`` pulled from a *fresh* realm still reports ``[native code]`` for a
   hooked function.

   These tests were **newly added** and have **not yet run against a real
   Firefox + xpi build or in CI**. Their result is therefore **unverified** — do
   **not** read this subsection as proof the frame-protection surface is clean.
   They are asserted-but-unverified.

   The assertions are also **test-bounded by the treadmill caveat**: a PASS only
   covers the same-origin realms the treadmill eagerly instruments, and does
   **not** prove safety against an *escape realm* the treadmill never reaches. A
   FAIL, conversely, is a genuine Wall-1-class leak. Neither outcome is claimed
   here in advance.
