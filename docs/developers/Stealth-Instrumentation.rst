...
1. **Native ``toString()``** is preserved via Firefox's privileged
   ``exportFunction``, with a ``toString`` proxy so wrappers report
   ``[native code]``.
...
instance, walks one step to ``Window.prototype``, and instruments the native
accessors in place. Because the descriptor is replaced on the prototype where the
browser natively defines it — with ``exportFunction``-backed getters/setters that
report ``[native code]`` — no own property is added to the ``window`` instance and
the masquerade is preserved (this is why instrumenting ``window.name`` is
