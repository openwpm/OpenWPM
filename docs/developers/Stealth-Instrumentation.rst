...
- ``toString()`` **leak** — wrapped functions return their JS body instead of
  ...
  **inherent**: a page can reach a *sibling realm's* ``Function.prototype.toString``
  ...
  :doc:`ADR 0001 <adr/0001-retain-legacy-js-instrument>`.
- **Prototype pollution** — properties from ancestor prototypes are flattened
  ...
  **removable but costly, not a second structural wall**: the flatten is
  ...
  data-layout and attribution-granularity cost. ADR-0001 states it this way; if
  you find wording anywhere that calls prototype pollution inherent, the ADR is
  the correct version.
...
Xray machinery stops the define. Earlier revisions of this document claimed that
``JSXrayTraits::defineProperty`` *forbids* defining an accessor on an
``[Object]``/``[Array]`` Xray instance; that claim was never backed by an
experiment and has been retracted. Treat the ceiling as a choice with a stated
reason, which is how ``config.py`` and ADR-0001 state it.

The one alternative that would have avoided the observable mutation *was*
prototyped, and it failed:
