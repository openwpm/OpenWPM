...
class LocalListener(BaseListener):
    ...
    def process_record(self, record: Tuple[str, Union[str, Dict[str, Any]]]):
        ...
        statement, args = self._generate_insert(table=table, data=data)
    ...
    def process_content(self, record):
        ...
        if table != RECORD_TYPE_CONTENT:
            raise ValueError(
                ...
                "record of type `%s`, received `%s`." % (RECORD_TYPE_CONTENT, table)
            )
...
class LocalAggregator(BaseAggregator):
    ...
    def save_configuration(self, openwpm_version, browser_version):
        ...
        self.cur.execute(
            ...
            "VALUES (?,?,?)",
            (
                json.dumps(self.manager_params.to_dict()),
                openwpm_version,
                browser_version,
            ),
        )
        ...
        for i in range(self.manager_params.num_browsers):
            self.cur.execute(
                ...
                (
                    ...
                    self.task_id,
                    json.dumps(self.browser_params[i].to_dict()),
                ),
            )
        self.db.commit()
