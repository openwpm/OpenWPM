...
class CrawlSubpagesCommand(BaseCommand):
    """
    Reliably crawl N internal subpages by extracting hrefs and loading them
    via webdriver.get(), instead of clicking on elements.
    """

    def __init__(self, url, max_pages=5, delay=4):
        self.url = url
        self.max_pages = max_pages
        self.delay = delay

    def __repr__(self):
        return f"CrawlSubpagesCommand({self.url},{self.max_pages},{self.delay})"
    ...
    def execute(
        ...
        # Load the base page
        base = GetCommand(self.url, self.delay)
        base.set_visit_browser_id(self.visit_id, self.browser_id)
        base.execute(webdriver, browser_params, manager_params, extension_socket)

        root = self.url.rstrip("/")

        # Extract internal links
        raw_links = get_intra_links(webdriver, root)
        hrefs = []
        for el in raw_links:
            try:
        ...
        clean_links = list(dict.fromkeys(hrefs))[: self.max_pages]

        logger.info(f"[{self.browser_id}] Found {len(clean_links)} subpages to visit.")

        # Visit each link via GET
        for link in clean_links:
            try:
                logger.info(f"[{self.browser_id}] Loading subpage: {link}")

                webdriver.get(link)
                wait_until_loaded(webdriver, 60)
                ...
                logger.error(
                    f"[{self.browser_id}] Error visiting subpage {link}: {e}",
                    exc_info=True
                )
                continue
