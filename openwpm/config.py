    ...
    try:
        ...
        if (
            ...
            raise ConfigError(
                ...
                "supported under stealth_js_instrument. Recursion requires "
                "mutating the page's in-page object graph (defining accessors on "
                "the Object/Array instances returned by instrumented getters), "
                "which is fundamentally incompatible with stealth's isolated "
                "compartment — the very isolation that makes it undetectable. "
                "Firefox's Xray wrappers forbid defining accessors on Object/Array "
                "Xray instances, and waiving to wrappedJSObject yields a different "
                "object identity, so page-side reads never see the instrumentation. "
                "Use js_instrument for configurations that need recursive, or "
                "remove recursive from your stealth_js_instrument_settings."
            )
