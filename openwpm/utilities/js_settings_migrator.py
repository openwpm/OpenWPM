...
def _launch_browser(firefox_binary: Union[str, Path]) -> webdriver.Firefox:
    ...
    descends at ``document_start``. Privileged parent-process access is requested
    the same way ``deploy_browsers`` does it: via geckodriver's
    ``--allow-system-access`` command-line flag. It must not be passed as
    Firefox's ``-remote-allow-system-access`` argument, because geckodriver
    0.37.1+ rejects that when it arrives through capabilities (a remote client
    granting itself system access would be a privilege escalation).
