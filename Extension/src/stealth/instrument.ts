...
function getPathToDomElement(
  element: any,
  visibilityAttr: boolean = false,
): string | undefined {
 ...
 */
function getPropertyKeysToOverwrite(item: any) {
  const res: any[] = [];
...
function getContextualPrototypeFromString(context: any, objectAsString: any) {
 ...
 */
function getObjectProperties(context: any, item: any) {
  ...
  if (propertiesToInstrument === undefined || !propertiesToInstrument.length) {
    ...
    //
    // Each entry is copied into a FRESH object with a FRESH propertyNames
    // array. `item.logSettings.propertiesToInstrument` is a shared reference
    // into the module-level settings default (settings.ts), returned unchanged
    // by resolveInstrumentationSettings() on every frame. Since every frame
    // self-instruments from that same object, mutating the entries in place
    // (assigning `.object` below, or growing `.propertyNames` in the coalescing
    // loop) would contaminate the shared default with live DOM-prototype
    // references and merged property lists across frames. Working on copies
    // keeps the shared default pristine.
...
function changeProperty(
  ...
  // Own-vs-inherited guard for the FOOTGUN above. `descriptor` may have been
  // resolved by getPropertyDescriptor walking UP pageObject's prototype chain,
  // but Object.defineProperty installs it as an OWN property of pageObject. If
  // the property exists on the chain yet is NOT own on pageObject, making it
  // own would diverge hasOwnProperty/ownKeys/getOwnPropertyDescriptor from
  // native — a page-observable tell. Skip (rather than silently create the
  // divergence) and surface the misconfigured `depth`. A name that exists
  // NOWHERE on the chain is an intentional non-existing-property injection and
  // is left to install as own.
  if (
    name in pageObject &&
    !Object.prototype.hasOwnProperty.call(pageObject, name)
  ) {
    console.error(
      "OpenWPM stealth: refusing to instrument inherited property '" +
        name +
        "' as an own property — the configured `depth` resolved to an object " +
        "that does not natively own it, which would be page-detectable. " +
        "Fix the entry's `depth` to land on the owning object.",
    );
    return;
  }
...
function functionGenerator(
  ...
  function temp(this: any) {
    ...
    try {
      ...
      const fakeError = generateErrorObject(err as any);
      throw fakeError;
    }
    return result;
  }
...
function needsWrapper(object: any) {
