...
# ---------------------------------------------------------------------------
# I2 under a *permanent* provider outage -- shutdown must still terminate
# ---------------------------------------------------------------------------
@pytest.mark.asyncio
@pytest.mark.pyonly
async def test_shutdown_terminates_when_flush_permanently_fails() -> None:
    """A backend that never recovers must not wedge shutdown.

    When the shutdown drain flush fails on every attempt, the finalize tokens
    never resolve. Shutdown must NOT await them forever: it has to give up,
    enqueue every observed visit as terminal (success=False), and return so the
    controller process exits instead of leaking. ``run_shutdown`` wraps the
    drain in a hard 30s timeout, so a regression to the old hang surfaces here
    as a ``TimeoutError``.
    """
    # Fail every write_table call, exhausting the shutdown retry budget.
    provider = RecordingArrowProvider(fail_writes=10**9)
    driver = ControllerDriver(provider)
    await driver.init()
    ...
    for visit_id in visit_ids:
        await driver.emit(SITE_VISITS, visit_id, site_visit_record(visit_id))
        await driver.settle()
    # Mix of finalized and never-finalized visits, all pending at shutdown.
    await driver.finalize(1, success=True)
    await driver.finalize(2, success=False)
    # visit 3 is never finalized

    # Must return (not hang) despite the permanent flush failure.
    await driver.run_shutdown()
    completed = dict(driver.drain_completion_queue())

    for visit_id in visit_ids:
        assert visit_id in completed, (
            f"visit {visit_id} was not enqueued as terminal when the flush "
            f"permanently failed -- shutdown stranded it"
        )
        assert completed[visit_id] is False, (
            f"visit {visit_id} reported success={completed[visit_id]} after a "
            f"permanent flush failure"
        )
    # Nothing could be committed, but the drain still gave up cleanly.
    assert provider.write_calls >= SHUTDOWN_FLUSH_RETRIES
