...
class Browser:
    ...
    def launch_browser_manager(self):
        ...
        while not success and unsuccessful_spawns < self._UNSUCCESSFUL_SPAWN_LIMIT:
            self.logger.debug(
                "BROWSER %i: Spawn attempt %i " % (self.crawl_id, unsuccessful_spawns)
            )
            ...
            try:
                ...
                self.logger.error(
                    "BROWSER %i: Spawn unsuccessful %s" % (self.crawl_id, error_string)
                )
    ...
    def kill_browser_manager(self):
        ...
        if self.display_pid is not None:
            try:
                ...
                self.logger.debug(
                    "BROWSER %i: Display process does not " "exit" % self.crawl_id
                )
                pass
            except TypeError:
                self.logger.error(
                    "BROWSER %i: PID may not be the correct "
                    "type %s" % (self.crawl_id, str(self.display_pid))
                )
        ...
        if self.browser_pid is not None:
            ...
            try:
                ...
                self.logger.debug(
                    "BROWSER %i: Geckodriver process already " "killed." % self.crawl_id
                )
                pass
            except psutil.TimeoutExpired:
                self.logger.debug(
                    "BROWSER %i: Timeout while waiting for "
                    "geckodriver or browser process to close " % self.crawl_id
                )
                pass
