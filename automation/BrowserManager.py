...
class Browser:
    ...
    def ready(self):
        ...
        return (
            self.command_thread is None or not self.command_thread.is_alive()
        )
    ...
    def launch_browser_manager(self):
        ...
        while (
            not success
            and unsuccessful_spawns < self._UNSUCCESSFUL_SPAWN_LIMIT
        ):
            self.logger.debug(
    ...
    def kill_browser_manager(self):
        ...
        if self.display_pid is not None:
            try:
                ...
                self.logger.debug(
                    "BROWSER %i: Display process does not "
                    "exit" % self.crawl_id
                )
                pass
            except TypeError:
